...
class UIManager:
    ...
    def update_display(
        ...
        self._add_stats_panel(
            display_frame, total_count, punch_counts, session_start_time, sensitivity
        )
    ...
    def _add_stats_panel(
        self, frame, total_count, punch_counts, session_start_time, sensitivity
    ):
    ...
    def _add_paused_overlay(self, frame):
        ...
        overlay = frame.copy()
        cv2.rectangle(
            overlay, (0, int(h / 2 - 40)), (w, int(h / 2 + 40)), (0, 0, 0), -1
        )
        cv2.addWeighted(overlay, 0.6, frame, 0.4, 0, frame)
        cv2.putText(
            frame,
            "PAUSED",
            (int(w / 2) - 60, int(h / 2) + 10),
            self.font,
            1.2,
            (0, 0, 255),
            3,
    ...
    def generate_stats_graph(self, historical_data, current_session_data):
        ...
        if historical_data:
            dates = [
                session["date"] for session in historical_data[-5:]
            ]  # Last 5 sessions
            total_punches = [
                session["total_punches"] for session in historical_data[-5:]
            ]
            ppm_values = [
                session["punches_per_minute"] for session in historical_data[-5:]
            ]
