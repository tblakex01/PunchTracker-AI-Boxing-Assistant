...
class Calibrator:
    ...
    def _collect_calibration_data(self, keypoints):
        ...
        for hand_key in ["left_wrist", "right_wrist"]:
            ...
            if keypoint is not None:
                ...
                self.punch_velocities.append(
                    keypoint[2]
                )  # Using confidence as a proxy for now
    ...
    def _process_calibration_data(self):
        ...
        if self.punch_velocities:
            ...
            self.calibration_data["velocity_multiplier"] = max(
                0.5, min(2.0, self.calibration_data["velocity_multiplier"])
            )
    ...
    def _draw_calibration_status(self, frame):
        ...
        if self.calibration_stage < len(self.calibration_steps):
            ...
            remaining_time = max(
                0, self.step_durations[self.calibration_stage] - elapsed_time
            )
            ...
            # Add progress bar
            progress_width = int(
                w * (elapsed_time / self.step_durations[self.calibration_stage])
            )
